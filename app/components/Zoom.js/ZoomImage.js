...
export function ZoomImage(props) {
  ...
  const gesture = useMemo(() => {
    const resetZoomState = () => {
      ...
      const duration = 450;
      // reset all state
      translationX.value = withTiming(0, { duration: duration });
      translationY.value = withTiming(0, { duration: duration });
    ...
    const doubleTap = Gesture.Tap()
      .onStart((e) => {
        ...
        if (scale.value !== 1) {
          ...
          scale.value = withTiming(doubleTapMaxZoomScale, {
            duration: tapDuration,
          });
          translationX.value = withTiming(
            -1 * (doubleTapMaxZoomScale * (e.x - viewWidth.value / 2)),
            { duration: tapDuration }
          );
          translationY.value = withTiming(
            -1 * (doubleTapMaxZoomScale * (e.y - viewHeight.value / 2)),
            { duration: tapDuration }
          );
        }
      })
      .numberOfTaps(2);
    if (
      zoomListContext === null || zoomListContext === void 0
        ? void 0
